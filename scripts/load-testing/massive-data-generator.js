...
class MassiveDataGenerator {
  ...
  async generateUsers(count, workerId) {
    ...
    for (let i = 0; i < count; i++) {
      const name = faker.helpers.arrayElement(indianNames);
  ...
  async generateRealisticPurchases(count, offers, users) {
    ...
    for (let i = 0; i < Math.min(count, availableOffers.length); i++) {
      ...
      purchases.push({
        ...
        amount: Math.round(offer.price * faker.number.float({ min: 0.95, max: 1.05 })),
        status: faker.helpers.weightedArrayElement([
        ...
        deliveryAddress: `${faker.location.streetAddress()}, ${faker.helpers.arrayElement(indianCities)}`,
        buyerDetails: {

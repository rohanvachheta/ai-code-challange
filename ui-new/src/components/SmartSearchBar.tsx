...
export function SmartSearchBar({
  ...
  const fetchSuggestions = useCallback(async (searchQuery: string) => {
    ...
    if (searchQuery.trim() === '*') {
      console.log('🌐 Global search detected: "*" - showing global search suggestion');
      const globalSuggestion: SmartSuggestion = {
        id: 'global-search',
        text: '*',
        highlightedText: '<strong>*</strong> - Show all records for ' + userContext.userType,
        confidence: 1.0,
        suggestionType: 'name', 
        context: `Retrieve all ${userContext.userType} records based on account ID and description`
      };
  ...
  const getSmartPlaceholder = () => {
    if (!query) {
      return 'AI Search: VIN, make/model, phone, location, or "*" for all records...';
    }
    
    if (query.trim() === '*') {
      return `Global search: Show all ${userContext.userType} records...`;
    }
